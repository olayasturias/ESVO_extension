...
esvo_core::core::EventBM::EventBM(
    ...
    bool bUpDownConfiguration) : camSysPtr_(camSysPtr), sb_(Sobel(3)), NUM_THREAD_(numThread),
    ...
    esvo_core::core::EventMatchPair &emPair)
{
  ...
  if (camSysPtr_->cam_left_ptr_->UndistortRectify_mask_.coeff(x_rect(1), x_rect(0)) <= 125)
    return false;
  ...
  if (!isValidPatch(x1, x1_left_top))
    return false;
  // LOG(INFO) << x1_left_top(1) << " " << x1_left_top(0) << " " 
  //           << patch_size_Y_ << " " << patch_size_X_ << " " 
  //           << pStampedTsObs_->second.TS_left_.rows() << " " << pStampedTsObs_->second.TS_left_.cols();
